  ...
  .transform((raw, ctx) => {
    ...
    if (Number.isNaN(parsed)) {
      ctx.addIssue({
        ...
        message: "monthlyOverpaymentLimit must be numeric",
      });
      return z.NEVER;
    }
    return parsed;
  ...
  .pipe(
    z
      ...
      .refine(
        ...
        "monthlyOverpaymentLimit must be a finite number",
      )
      .refine(
        (value) => value >= 0,
        "monthlyOverpaymentLimit must be greater than or equal to 0",
      )
      .refine(
        (value) => value <= MAX_MONTHLY_OVERPAYMENT_LIMIT,
        "monthlyOverpaymentLimit exceeds the allowed maximum",
      )
      .refine(
        hasAtMostTwoDecimalPlaces,
        "monthlyOverpaymentLimit must not include more than two decimal places",
      ),
  );
...
export const userSettingsUpdateSchema = z
  .object({
    ...
    reinvestReducedPayments: z.boolean({
      invalid_type_error: "reinvestReducedPayments must be a boolean",
    }),
  })
  .strict();

const mapIssuesToErrorCode = (
  issues: z.ZodIssue[],
): { code: string; message: string } => {
  if (issues.some((issue) => issue.code === z.ZodIssueCode.unrecognized_keys)) {
    return {
      code: "EXTRANEOUS_PROPERTY",
      message: "Request body contains unsupported properties",
    };
  }
  ...
  if (
    ...
    return {
      code: "NEGATIVE_LIMIT",
      message: "monthlyOverpaymentLimit must be greater than or equal to 0",
    };
  }
  ...
  if (
    issues.some(
      (issue) =>
        issue.code === z.ZodIssueCode.invalid_type && issue.path.length === 0,
    )
  ) {
    return {
      code: "INVALID_BODY",
...
export const parseUserSettingsUpdate = (
  input: unknown,
