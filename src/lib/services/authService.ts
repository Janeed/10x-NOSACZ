...
export async function signIn(
  ...
  try {
    ...
    throw internalError('SUPABASE_UNAVAILABLE', 'Signin service unavailable', { cause });
  }
  ...
  if (error) {
    ...
    if (error instanceof AuthApiError) {
      if (error.status === 429) {
        throw tooManyRequestsError('RATE_LIMITED', 'Too many signin attempts. Try again later.', details);
      }

      const invalidCredentials = /invalid (login )?credentials/i.test(error.message);
      if ((error.status === 400 || error.status === 401) && invalidCredentials) {
        throw unauthorizedError('INVALID_CREDENTIALS', 'Invalid email or password', details);
      }
      ...
      if (error.status >= 400 && error.status < 500) {
        throw validationError('VALIDATION_ERROR', 'Signin request rejected', details);
      }
    }

    throw internalError('SUPABASE_ERROR', 'Signin failed', { details, cause: error });
  }

  if (!data.user?.id) {
    throw internalError('INCOMPLETE_RESPONSE', 'Signin response missing user identifier');
  }

  if (!data.user.email) {
    throw internalError('INCOMPLETE_RESPONSE', 'Signin response missing user email');
  }

  if (!data.session?.access_token || !data.session.refresh_token) {
    throw internalError('INCOMPLETE_RESPONSE', 'Signin response missing session tokens');
  }
