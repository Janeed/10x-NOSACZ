...
export const GET: APIRoute = async ({ locals, request }) => {
  ...
  try {
    ...
    logger.error("monthly_execution_logs_get", "Failed to get monthly execution logs", {
...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  try {
    ...
    const result = await createLog(cmd, locals.supabase, userId, requestId || "");
    ...
    logger.error("monthly_execution_logs_post", "Failed to create monthly execution log", {
      userId: locals.userId,
      requestId,
      error: error instanceof Error ? error.message : String(error),
    });
