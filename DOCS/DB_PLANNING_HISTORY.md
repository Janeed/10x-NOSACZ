...
jsmieszek: 1. where is 1?
2. yes, in order to keep track of past progress, the simulation starts from the change point always.
3. let's recalculate on demand for now
4. good idea, let's add status and isActive columns
5. your idea with monthly_execution_logs sounds ok
...
4. good idea, let's add status and isActive columns
5. your idea with monthly_execution_logs sounds ok
6. let's assume strategy type will be string column which will be represented as enum on code-side. so hardcoded for now for simplicity
7. enable RLS
8. no, keep it simple
9. no
10. let's not worry about it for MVP
...
jsmieszek: 2. each loan row should contain data from first register (amounts, interest, payments remaining) and based on that we can simulate baseline. would that be enough?
3. let's store the simulation snapshots
4. proceed with the recommendation
5. recommendation sounds about right!
6. no need for soft delete
7. sounds good, let's keep loan change events
8. there will be minimal amount of data for MVP, let's skip indexes
9. enforce user_id = auth.uid()
10. let's just implement debounce on UI for now in such cases
