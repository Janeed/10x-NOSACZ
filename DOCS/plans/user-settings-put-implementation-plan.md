  ...
  .object({
    monthlyOverpaymentLimit: z
      ...
      .refine(
        ...
        "monthlyOverpaymentLimit must have at most 2 decimal places",
      ),
...
export async function upsertUserSettings(
  ...
  if (existingRes.error)
    throw internalError("SUPABASE_ERROR", "Failed to read user settings", {
      cause: existingRes.error,
    });
  ...
  if (!existing) {
    ...
    if (insertRes.error || !insertRes.data)
      throw internalError("SUPABASE_ERROR", "Insert failed", {
  ...
  if (ifMatch && ifMatch !== existing.updated_at) {
    throw conflictError(
      ...
      "Settings modified by another process",
    );
  }
  ...
  if (updateRes.error)
    throw internalError("SUPABASE_ERROR", "Update failed", {
